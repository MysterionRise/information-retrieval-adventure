...
public class BM25FSimilarity extends Similarity {
  ...
  /**
   * The default implementation computes the average as <code>sumTotalTermFreq / maxDoc</code>, or
   * returns <code>1</code> if the index does not store sumTotalTermFreq (Lucene 3.x indexes or any
   * field that omits frequency information).
   */
  public float avgFieldLength(CollectionStatistics collectionStats) {
   ...
   * </pre>
   *
   * <p>Note that {@link CollectionStatistics#maxDoc()} is used instead of {@link
   ...
   * CollectionStatistics#maxDoc()}, and in the same direction. In addition, {@link
   * CollectionStatistics#maxDoc()} is more efficient to compute
   *
  ...
  public final SimWeight computeWeight(
    ...
    return new BM25FStats(collectionStats.field(), idf, queryBoost, avgdl, cache);
  }
  ...
  private Explanation explainScore(
      int doc, Explanation freq, BM25FStats stats, NumericDocValues norms) {
