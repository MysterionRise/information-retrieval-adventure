...
public class CustomScoring {
  ...
  public static void main(String[] args) throws IOException, ParseException {
    ...
    doc.add(
        new TextField(
            "text", "i would be curious how bubble tea is bubble enough for me", Field.Store.YES));
    writer.addDocument(doc);
    doc = new Document();
    doc.add(
        new TextField(
            "text",
            "i would be curious how bubble tea is bubble enough for me. Should I have more bubble ?",
            Field.Store.YES));
    writer.addDocument(doc);
    writer.close();
    ...
    Query query =
        new BooleanQuery.Builder()
            .add(
                new ConstantScoreQuery(new TermQuery(new Term("text", "bloom"))),
                BooleanClause.Occur.SHOULD)
            .add(
                new ConstantScoreQuery(new TermQuery(new Term("text", "bubble"))),
                BooleanClause.Occur.SHOULD)
            .build();
